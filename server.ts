...
// -----------------------------------------------------------------------------
// Payload Generation (SSH direct over TCP -- HTTP CONNECT style payload)
// -----------------------------------------------------------------------------
function getSshTcpPayloadText(domain: string): string {
  return `CONNECT ${domain}:443 HTTP/1.1[crlf]Host: ${domain}[crlf][crlf]`;
}
...
const MAIN_REPLY_KEYBOARD = {
  keyboard: [
    [{ text: "🔐 بيانات اتصال SSH" }, { text: "🌐 بايلود TCP" }],
...
async function sendMainMenu(chatId: number | string) {
  ...
  const text =
    ...
    `🌐 <b>SSH مباشر عبر TCP (بايلود CONNECT):</b> اتصال مباشر بدون WebSocket.\n\n` +
    `اختر من الأزرار بالأسفل لإدارة الخادم أو نسخ بيانات الاتصال:`;

  const inlineKeyboard = [
    [{ text: "🌐 بايلود TCP", callback_data: "send_payload_tcp" }, { text: "🔐 بيانات اتصال SSH", callback_data: "cfg_ssh" }],
...
async function sendSshInfo(chatId: number | string) {
  ...
  const locationText = location ? `${location.flag} ${location.countryName}` : "🌍 غير معروف";
  const locationDetail = location ? formatServerLocationDetail(location) : "";
  const tcpPayload = getSshTcpPayloadText(domain);

  const text =
    `🔐 <b>بيانات اتصال سيرفر Moon SSH (TCP مباشر):</b>\n\n` +
    `🟢 <b>حالة السيرفر:</b> ${running ? "يعمل بنجاح 🟢" : "متوقف 🔴"}\n` +
    `🗺️ <b>موقع السيرفر:</b> ${locationText}\n` +
    (locationDetail ? `${locationDetail}\n` : "") +
    ...
    `👤 <b>اسم المستخدم:</b> <code>${escapeHtml(creds.username)}</code>\n` +
    `🔑 <b>كلمة المرور:</b> <code>${escapeHtml(creds.password)}</code>\n\n` +
    `🌐 <b>بايلود TCP (CONNECT):</b>\n` +
    `<code>${escapeHtml(tcpPayload)}</code>`;

  const inlineKeyboard = [
    [{ text: "🌐 بايلود TCP", callback_data: "send_payload_tcp" }, { text: "🏠 القائمة الرئيسية", callback_data: "main_menu" }],
    [{ text: "👤 تغيير المستخدم", callback_data: "ssh_change_username" }, { text: "🔑 تغيير كلمة المرور", callback_data: "ssh_change_password" }]
  ];
...
async function sendTcpPayloadOnly(chatId: number | string) {
  const domain = getPublicDomain();
  const tcpPayload = getSshTcpPayloadText(domain);

  const text =
    `🌐 <b>بايلود بروتوكول TCP المباشر لـ Moon SSH:</b>\n\n` +
    `<code>${escapeHtml(tcpPayload)}</code>\n\n` +
    `💡 <i>السيرفر يقبل اتصال CONNECT ويمرر نفق الـ SSH مباشرة عبر TCP بدون WebSocket!</i>`;
...
async function sendServerStatus(chatId: number | string) {
  ...
  const statusText = `📊 <b>حالة خادم Moon SSH:</b>\n\n` +
    `🟢 <b>الخدمة:</b> Moon SSH TCP مباشر / TLS-SNI (Dropbear Engine)\n` +
  ...
  try {
    await telegramApi("setMyCommands", {
      commands: [
        ...
        { command: "tcp", description: "🌐 بايلود TCP" },
...
async function handleTelegramUpdate(update: any) {
  if (update.message) {
    ...
    if (text === "/tcp" || text === "/payload_tcp" || text === "/payload" || text === "🌐 بايلود TCP" || text === "بايلود TCP" ||
        text === "📄 نسخ الـ Payload" || text === "📄 الـ Payload" || text === "Payload" ||
        textLower === "tcp" || textLower === "payload") {
      delete userSessions[chatId];
      await sendTcpPayloadOnly(chatId);
      return;
    }
...
// NOTE: SSH tunneling is no longer bridged as a WebSocket upgrade inside
// Express middleware. Node's http.Server intercepts HTTP CONNECT-method
// requests before they ever reach Express (it emits a "connect" event on
// the raw server instead of the normal "request" event), so the TCP/CONNECT
// tunnel bridge is registered directly on `server` further down, right next
// to `server.listen(...)`.
...
app.get("/", (req, res) => {
  ...
  if (botToken) {
    return res.json({
      status: "ok",
      service: "Moon SSH Server (Dropbear over TCP/TLS 443)",
      protocol: "SSH-TCP / SSL-SNI",
...
// -----------------------------------------------------------------------------
// HTTP Server & Reverse Proxy for Dropbear SSH (direct TCP / CONNECT only)
// -----------------------------------------------------------------------------
...
// Malformed / non-HTTP requests are rejected outright. Only a well-formed
// HTTP CONNECT request (the payload shown by the bot) is bridged -- see the
// "connect" handler below.
server.on("clientError", (_err: any, socket: net.Socket) => {
  try { socket.destroy(); } catch {}
});

// CONNECT Handler: direct transparent TCP stream bridge to Dropbear SSH.
// The client sends "CONNECT host:port HTTP/1.1" (the payload from the bot);
// we reply "200 Connection Established" and from that point on the socket
// is a raw, unframed TCP pipe carrying the SSH protocol directly -- no
// WebSocket framing/handshake involved.
server.on("connect", (req, socket, head) => {
  addLog(`[Connect-Bridge] Intercepted CONNECT ${req.url}, responding 200 Connection Established & bridging`);
